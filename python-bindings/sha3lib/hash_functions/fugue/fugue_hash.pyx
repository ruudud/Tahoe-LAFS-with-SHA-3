...
def hash(int hashbitlen, bytes data, int in_length):
    ...
    cdef fugue_hash_h.DataLength datalen = in_length
    cdef fugue_hash_h.BitSequence *hashval = <fugue_hash_h.BitSequence *> malloc(hashbitlen*8)

    fugue_hash_h.Hash(hashbitlen, <fugue_hash_h.BitSequence *> c_string, datalen, hashval)
...
cdef class fugue:
    ...
    scratch.
    '''
    cdef fugue_hash_h.BitSequence *hashval
    cdef fugue_hash_h.hashState previous_state
    cdef fugue_hash_h.hashState state
    cdef int finished
    cdef int hashbitlen
    ...
    def __init__(self, int in_hashbitlen, bytes initial=None):
        ...
        fugue_hash_h.Init(&self.state, self.hashbitlen)
    ...
    cpdef update(self, bytes in_data):
        ...
        fugue_hash_h.Update(&self.state, <fugue_hash_h.BitSequence *> data, data_len)

    cpdef final(self):
        self.hashval = <fugue_hash_h.BitSequence *> malloc(self.hashbitlen*8)

        # We copy the state so that we can continue to update.
        # This equals hashlibs functionality, but not pycryptopp.
        self.previous_state = self.state

        fugue_hash_h.Final(&self.state, self.hashval)
        self.finished = 1
