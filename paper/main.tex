...
Their compiler flags used, which are extracted from the \ac{SUPERCOP}
results, can be seen in Table \ref{tbl:sha3:compilerflags}.

It should be noted that \emph{32} in blake32 does not refer to the output length
of the function, but the fact that the algorithm is optimized for 32-bit usage.
Similarly, \emph{512} in keccakc512 does not mean 512-bit output, but 512 cycles in the
internal function.
...
contacted the author, Thomas Pornin, which confirms this problem with the
official JH candidate and that the author has been made aware of the problem
but has not released any updated version yet (personal communication, October
16, 2010).
...
% Go graphs with plenty of short, descriptive comments.
...
If we assume that cycles/bytes is linear from 8 byte to 64 byte, we get the
following estimate for the speed of hashing 32 bytes:

$$179.62 - \frac{179.62 - 38.12}{64 - 8} * (32-8)= 118.98 (Cycles/byte)$$

We can then estimate the result of each message by the following formula:
$$\frac{median * size + 118.98*32}{size} = SHA256d estimate$$

\input{tables/supercop_sha256d}
Estimates based on this calculation are shown in Table
\ref{tbl:supercop:sha256d}. What we can see is that for lower lengths of data,
more of the candidates are faster than double \ac{SHA}-256. However, given the
practical situation that Tahoe-\ac{LAFS} is a filesystem, most files processed
will have be larger than the file sizes estimated here. Even though not all
hashing operations done in Tahoe are related to the file size, we expect that
only BLAKE, \ac{BMW}, Luffa, Shabal and SIMD will be able to outperform double
\ac{SHA}-256.

\section{Validity of \ac{SUPERCOP} results}
Although we believe the \ac{SUPERCOP} toolkit and results give good indication
of how fast the different candidate implementations are, we do have som
comments on it.

\paragraph{Implementations does not conform to \ac{NIST} specification} for
many of the candidates. By this we mean that there is no Update()-function
available for all implementation, and that \ac{SUPERCOP} only measures speed of
hashing full bytestrings. This is also a hasle for programmer who want to use
the implementations in \ac{SUPERCOP}

\paragraph{Incorrect implementations} are present in \ac{SUPERCOP}. By this we
mean implementations that does not verify the \ac{KAT}s. 

\parapgraph{Results are only valid for one specific computer}. The results we
...
might be obtained from looking at the results from all computeres measured by
\ac{SUPERCOP}.

% This chapter should contain (in listed and grouped order):
% -----------------------------------------------------------
% Describe inconsitencies in the numbers, and try to explain them.
%
...
%
% More general stuff. What could have been done differently? Critisize
% ourselves.
