...
% TODO: How to reference email?
% TODO: We have to write someting about sphlib itself
...
who also supplied us with a patch to correct the bug.
...
Tahoe, but the copy()-function is a part of Hashlib. The idea is that if we
have two bytestring that share a common prefix, we can input this prefix to
Update(), then copy the state. The tests check if this is possible, and that a
copy of the state and following update does not affect the original state.
...
Immutable files are not tested, move text from Section 3.3.3.

\section{Measuring Performance}
Include code from hashutil.py and the use of time in bash.

\section{\ac{SUPERCOP} benchmarks}
% TODO: more introduction text here.

The results of \ac{SHA}-3 256-bit functions and \ac{SHA}-256 benchmarking in
...
faster the algorithms performed in the test.
...
%
% More general stuff. What could have been done differently?
