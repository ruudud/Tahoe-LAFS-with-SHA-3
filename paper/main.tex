...
outperform \ac{SHA}-256. However, given that Tahoe-\ac{LAFS} uses double
\ac{SHA}-256, this means that it will also have to hash another 32 bytes for
every message it hashes. Since \ac{SUPERCOP} does not have a benchmark for 32
byte messages, we will have to estimate the amount of cycles required for this.
If we assume a worst case scenario of hashing 32 bytes, where the cycles per
byte count is exactly the same as for 8 bit median, we get:
...
\ref{tbl:supercop:sha256d}. For shorter lengths of data,
more of the candidates are faster than double \ac{SHA}-256. Nevertheless, given
the practical situation that Tahoe-\ac{LAFS} is a file system, most files
processed will be larger than the file sizes estimated here. Not all hashing
operations in Tahoe-\ac{LAFS} are strictly dependant on the original file size,
which would mean that all candidates would have a chance of outperforming double
\ac{SHA}-256. This estimate is however worst case, which leads us to believe
that at least for larger input sizes (i.e. over 1536 bytes) only the five
fastest candidates will have a chance of outperforming double \ac{SHA}-256.

\section{Measurements of Tahoe data}
% TODO: Fiks denne tabellen så den ikke overflower
% TODO: Sleng inn samme for download
Table \ref{tbl:tahoe:datauploaded} and describes the distribution of the
different input sizes of data that Tahoe-\ac{LAFS} sends to the hash function
when uploading and downloading the test vectors. E.g. to upload the 100 1-byte
files, 15700 hashes had to be calculated, and about 64\% of these were of size
106 byte.

For every upload Tahoe-\ac{LAFS} does, it hashes a size close to the original
file size twice. This does not happen on downloading the file.   
...
calculate the hash operations are exhibited in Figures \ref{fig:graph:10001b},
...
BLAKE, \ac{BMW}, Shabal and Simd outperforms \ac{SHA}-256 when both uploading
and downloading a 1GB file, but on small files, SHA-256 is faster by a large
margin. The difference between the slowest (JH) and the fastest (BLAKE)
algorithm is 229.92s when uploading the one gigabyte file.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashinguploading10001Bfiles.pdf}
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashingdownloading10001Bfiles.pdf}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashinguploading1001KBfiles.pdf}
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashingdownloading1001KBfiles.pdf}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashinguploading501MBfiles.pdf}
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashingdownloading501MBfiles.pdf}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashinguploading5100MBfiles.pdf}
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashingdownloading5100MBfiles.pdf}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashinguploading11GBfile.pdf}
    \includegraphics[width=0.9\columnwidth]{graphs/Timespenthashingdownloading11GBfile.pdf}
...
chunks being hashed is what is making the difference. 
