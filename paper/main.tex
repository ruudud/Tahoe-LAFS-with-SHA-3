...
mean implementations that does not conform to the \ac{KAT}s supplied or
...
manages to outperform is expected from the \ac{SUPERCOP} results, REF, however
the order and proportion of the measurements does not. This is primarily
because we have not used the fastest implementation for all candidates, and the
relative speed of the chosen implementation compared to the fastest
implementation in \ac{SUPERCOP} differs (Table \ref{tbl:sha3:relativespeed}.
...
implementations in \ac{SUPERCOP}. The results can be seen in appendix
\ref{appendix:optimzed_sha3}.
