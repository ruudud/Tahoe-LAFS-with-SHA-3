...
The generated C files from Cython used as Python bindings are not necessarily the most
optimized versions possible to write and they are ugly to read and change (from
a C perspective). However they are equal for all the \ac{SHA}-3 candidates
which means they should give the same performance penalty. The \ac{SHA}-256
version will of course not suffer from the same penalty. From
\autoref{tbl:cython:overhead} we can see the performance penalty of our
bindings, and we do believe better bindings can be made, also with the use of
Cython, but by someone with more skill and knowledge than us.
...
calculations uses medians from \autoref{app:supercop:bench} for the speed
of each candidate, the exact sizes for each call to the hash function from
\autoref{sec:results:tahoedata}, and interpolation to estimate the time to
...
this is probably due to interpolation being an inaccurate method to use for
