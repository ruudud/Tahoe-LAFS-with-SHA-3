...
every message it returns the digest for. Since \ac{SUPERCOP} does not have
benchmark for 32 byte messages, we will have to estimate the amount of cycles
required for this.  If we assume that cycles/bytes is linear from 8 byte to 64
byte, we get the following estimate for the speed of hashing 32 bytes:
...
$$\frac{median * size + 118.98*32}{size} = SHA256d$$ \emph{estimate}
...
will be larger than the file sizes estimated here. Even though not all
...
hash functions, one have to add a new capability (Zooko O'Whielacronx, personal
communication, September 29, 2010). This ensures backwards compatibility and can
be an optional feature for users to employ. However, we found that adding a new
capability was out of scope for this assignment.
...
% discussion
%
% We had hoped to have contributed more to the Open Source project that
% Tahoe-LAFS is.
