...
double \ac{SHA}-256. However this estimate is worst case, which leads us to
believe that at least for larger input sizes (over 1536) only the five fastest
...
For small files, 1B and 1KB, we can clearly see that both \ac{SHA}-256 and
double \ac{SHA}-256 greatly outperforms the \ac{SHA}-3 candidate
implementations, which is unexpected if the time Tahoe-\ac{LAFS} do mainly
realy on the filesize.

\subsection{Large inputs}
...
performing SHA-3 candidate, BLAKE, were to be used.
% TODO: Bedre og bedre implementasjoner kommer til å komme for SHA-3
