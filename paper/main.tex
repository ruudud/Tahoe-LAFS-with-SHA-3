...
\begin{figure}[!h]
    ...
    \emph{Figure based on
     ...
     {CHK-hashes.svg from tahoe-lafs.org}}
...
While the official \ac{NIST} \ac{SHA}-3 second round submissions contains both
reference and different optimized version of the candidates does not appear to
be updateded since the submission deadline on September 15, 2009. They also do
not contain any benchmarking util which could be used to decide on the best
implementation for a particular plattform or architecture. \ac{SUPERCOP} on the
other hand provides both, in combination with results stating which compiler
arguments gives the best performance. On the downside the \ac{SUPERCOP}
\ac{API} only specifies the crypto\_hash() function and not an Update()
function which Tahoe-\ac{LAFS} is dependant on. 

Since Tahoe-\ac{LAFS} uses \ac{SHA}-256 internally, we use only \ac{SHA}-3
...
We then chose the fastest implementation of each candidate that conforms to the
following criterias.
\begin{enumerate}
    \item The implementation must have an \ac{API} defined in C. (Not only
    assembly)
    \item The implementation must include a working Update() function.
    \item The implementation must not use assembly which can not be made
    position independant by gcc.
    \item The implementation should conform to the \ac{KAT}s specified in the
...
% This should be a short text, but maybe drag in a discussion of how we SHOULD
% have implemented it if we had the time, ref. capabilities.
