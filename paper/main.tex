...
The implementation we chose must also work after being compiled with the {\verb
-fPIC} flag in \ac{GCC}. This flag basically tells the compiler that the
machine code must be able to execute anywhere in memory, also known as position
independant. In runtime one extra \{CPU} register will be used for this. This
is needed because a Python C-module we will need to be compiled as a shred
library.
...
and the one used for every implementation which might affect performance, are:
...
implementation for the Echo candidate, the almost exact same pattern is used for
...
byte for all tests. For larger input sizes, Simd and Luffa also do the
