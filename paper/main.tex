...
The numbers for uploading and downloading the 1GB file are summarized in
...
and downloading a 1GB file. The difference between the slowest (JH) and the
fastest (BLAKE) algorithm is 229.92s when uploading the one gigabyte file.

The third column in Tables \ref{tbl:hashingtimes:put1gb} and
\ref{tbl:hashingtimes:get1gb} contains the time spent doing other operations
than hashing, i.e. $Total time - Time spent hashing$. This number varies
between 227.37s and 252.66s during upload and 205.94s and 225.03s during
download. \emph{Fraction} displays the part of total time the calls to the
hashing libraries take.
