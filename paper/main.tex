...
differences in performance. Only immutable file support was tested.
...
currently save up to $12,36\%$ on upload operations by changing the best 
...
\acro{SVN}{Subversion}
\acro{UEB}{\ac{URI} Extension Block}
...
algorithm is carried out by the {NIST}, and is in its final stages at the time
...
In this project assignment, we integrate the 14 second round \ac{SHA}-3
candidates in the Tahoe-\ac{LAFS} and measure the performance impact. To be
able to do this, we create Python bindings for the best suited C
implementations of the SHA-3 candidates, and alter the hash function calls in
the source code of Tahoe.
...
chosen between the submitted contestants to the \ac{NIST} hash competition.  At
the  time of writing, the competition is in its final round (round 3), with
only five remaining candidates. This result was not revealed until a week
before the submission of this paper, so we are focusing on all round 2
candidates, which there are 14 of \cite{s_fedreg}.
...
candidates is security \cite{s_nistround2}. \citet{s_fedreg} lays out the full
...
second most important criterion. The absolute minimum for performance is that
the SHA-3 candidate should be faster than the functions in the \ac{SHA}-2
family \cite{s_nistround2}.  Cost is a measure of how much memory an
implementation requires in software, both the implementation itself and the use
of \ac{RAM} during runtime. Another measure of cost is how many logic gates it
takes to implement the function in hardware.
...
\subsubsection{NIST SHA-3 API}

\ac{NIST} required that every
...
provide the actual data that will be hashed. The {\verb Final() } function should be
called when all necessary data has been given through {\verb Update() }, it will then
finish the processing and provide the digest. The {\verb Hash() } function
...
best results possible for each implementation.
...
architectures are available at the webpage of eBACS\footnote{\url{http://bench.cr.yp.to/results-sha3.html}}.

\subsubsection{SUPERCOP API}
...
of how long the output of the hash function should be.
...
framework, and can run on Windows, Mac OSX, Linux, Solaris and other operating
systems.
...
A \textbf{capability} (or an URI) contains the encryption key, and a hash of
the \ac{UEB}. The \ac{UEB} for each file is a data structure containing the
hash of the \ac{CEB}, the size of the file, any encoding parameters necessary
to perform the erasure decoding, and a hash of the plaintext and the encrypted
...
programming language. Python is also an just-in-time language, which means that
...
\begin{enumerate}
  ...
  \item Write Python bindings, with unit tests for verification, to be able to
    use the cryptographic hash functions from Python.
...
a reference version and optimized versions, they do not
appear to be updated since the submission deadline on September 15, 2009. They
do also not contain any benchmarking utility which could be used to decide on
...
compiler arguments giving the best performance. On the downside, the
...
The implementation we chose must also work after being compiled with the {\verb -fPIC }
flag in \ac{GCC}. This flag basically tells the compiler that the
machine code must be able to execute anywhere in memory, also known as position
independent. In runtime one extra \ac{CPU} register will be used for this. This
is needed because a Python C module we will need to be compiled as a shared
library.

Since Tahoe-\ac{LAFS} is using \ac{SHA}-256 internally, we only test \ac{SHA}-3
functions with 256-bit output to avoid problems that could be caused by longer 
hash values in Tahoe-\ac{LAFS}.
...
\paragraph{Criterion 3} The implementation must not use code that can not be made
...
candidates can be found at eBASH \cite{s_tahoe_supercop}.

\subsection{Chosen implementations}
\label{sec:chosen_implementations}
...
and the flags used for every implementation which might affect performance, are:
...
words, and output a 32-byte digest (256 bit). Similarly, \emph{512} in
keccakc512 does not mean 512-bit output, but 512 bits in the internal capacity.

\paragraph{Sphlib\protect\footnote{\url{http://www.saphir2.com/sphlib/}}}is a C and
Java library made by Thomas Pornin , which contains all the second round
\ac{SHA}-3 candidates. Some of the same implementations are also found in
\ac{SUPERCOP}, while others, like the Cubehash implementation we have used is
not.
...
from cryptopp\footnote{\url{http://www.cryptopp.com/}}, which is the one used by
...
this requirement. We have contacted the author of sphlib, Thomas Pornin, which confirms
...
who also supplied us with a patch to correct the bug \cite{s_simd}.
...
\begin{itemize}
    ...
    {\verb update() }
    \item {\verb update() } receives data that should be hashed
...
uses 256-bit output, in the end only support for 256-bit output in
...
for bringing the functionality of the \ac{SHA}-3 candidate implementation to Python. By compiling
...
\ref{lst:cython_impl}. Functions declared with {\verb def } are Python
functions, {\verb cdef } are Cython-only functions, while {\verb cpdef } are
functions that can be accessed from both languages. While this is just the
implementation for the Echo implementation, the almost exact same pattern is used for
...
build and install our code. The alternative of doing this would be to manually
...
Options and configuration for \ac{Distutils} is done through a file named
...
We Unit Test each new candidate implementation that is included in SHA3lib, both
to verify our own work and to verify that the implementations from \ac{SUPERCOP}
actually are correct.

\paragraph{Known-Answer Tests} are used to check that the implementation conform
to the specifications of the algorithm author. The \acp{KAT} include
byte strings, their length and their expected message digest. The length of the
...
\paragraph{Basic Hash Tests} are what we have named the set of tests which test
the core basics of what a hash function implementation will have to be able to
do to be usable. This include that {\verb init() }, {\verb update() } and
{\verb final() } works as expected, and that the output is of correct size. This
include that every function should produce the same digest for the inputs
{\verb update("foofoo") } and two consecutive calls to {\verb update("foo") }.

\paragraph{Copy Hash Tests} do not test any of the functionality used by
Tahoe-\ac{LAFS}, but the {\verb copy() } function is a part of hashlib, and we
have included it in SHA3lib to conform to this. The idea is that if we have two
byte strings that share a common prefix, we can input this prefix to
{\verb update() }, then copy the state. The tests check if this is possible, and
that altering the copied state does not affect the original.

\section{Modifications to the Tahoe-LAFS Code}
\label{sec:modtahoe}

The code of Tahoe-\ac{LAFS} conforms to the style guide of Python, known as PEP
8.  This, along with the code being highly modular and consistent to its
architecture, emphasizes code readability.  With some experience using Python,
it is not difficult to learn the Tahoe code enough to understand what is
happening when a file is uploaded or downloaded with the system.
...
function library is made in only one line. This is displayed in Listing
\ref{lst:hashutil}.

\lstinputlisting[label=lst:hashutil, caption=Extract from hashutil.py of Tahoe-LAFS source.]
{listings/hashutil.py}

As stated in \autoref{sec:lengthextension}, none of the \ac{SHA}-3
candidates are vulnerable to the length-extension attack. Since we are
interested in finding out how Tahoe-\ac{LAFS} performs when utilizing the
\ac{SHA}-3 functions, we can remove the lines that fixes the length-extension
problems manually. In addition, to try out one of the new implementations, we
change line one in Listing \ref{lst:hashutil}, and an example of this can be
seen in Listing \ref{lst:hashutilmod}.
...
\section{Automatic testing and distribution} We fully automate the testing
procedure. This is achieved by the use of software revision control systems,
which distributes both the source code of Tahoe-\ac{LAFS}, SHA3lib and
shell-scripts for installation and configuration of both. Configuration is
mainly done by the use of regular expressions.The outline of this procedure is:
...
As stated in \autoref{sec:modtahoe}, only the Tahoe-\ac{LAFS} file type
\emph{immutable files} are tested. The mutable files are not tested because of
our way of integrating the SHA-3 candidates within the Tahoe-LAFS code.
...
{\verb date } command in Bash to calculate the number of seconds it takes for
...
\ac{SUPERCOP} can be found in \autoref{app:supercop:bench}.

BLAKE, \ac{BMW}, and Luffa outperform \ac{SHA}-256 on cycles per
byte for all tests. For larger input sizes, Simd and Shabal also do the
calculations quicker than \ac{SHA}-256.
...
summarized in Tables \ref{tbl:hashingtimes:put1b}, \ref{tbl:hashingtimes:get1b},
\ref{tbl:hashingtimes:put1kb}, \ref{tbl:hashingtimes:get1kb},
\ref{tbl:hashingtimes:put1mb}, \ref{tbl:hashingtimes:get1mb},
\ref{tbl:hashingtimes:put100mb}, \ref{tbl:hashingtimes:get100mb},
\ref{tbl:hashingtimes:put1gb} and \ref{tbl:hashingtimes:get1gb}. The third
column contains the time spent doing other operations than hashing, i.e.
\emph{Time spent not hashing = Total time - Time spent hashing}. \emph{Fraction}
displays the part of total time the calls to the hashing libraries take.

\ac{SHA}-256 (single) is not included in the graphs, because the results were
almost the same as for the double \ac{SHA}-256 used in Tahoe-\ac{LAFS}.
...
Because we write to two files after each call to {\verb digest }, an overhead in
the measurements, that would not normally exist in Tahoe-\ac{LAFS}, will be
introduced. This overhead should be about the same for all candidates, since the
number of writes are equal. The results and calculations for this overhead,
based on runs with double \ac{SHA}-256, can be seen in Table
\ref{tbl:measure:overhead}.
...
\section{Reliability of SUPERCOP results}

Although we believe the \ac{SUPERCOP} toolkit results give good indication of
how fast the different implementations of the candidates are, we do have some
general comments on the way it works and how it relates to our results.

\paragraph{Implementations do not conform to \ac{NIST} specification} for
...
measures speed of hashing full byte strings. We are questioning if this
discriminates implementations which actually can do more of their processing on
a call to {\verb update() }. This issue is also a hassle for a programmer who
wants to use the implementations found in \ac{SUPERCOP}.

\paragraph{Incorrect implementations} are present in \ac{SUPERCOP}. By this we
mean implementations that does not conform to the \acp{KAT} supplied or
implementations which fail completely as hash functions, see
\autoref{sec:chosen_implementations} for details on the JH implementations.

\paragraph{Extra compiler flags.}

We do not compile the candidate implementation with the exact same arguments
that is used in \ac{SUPERCOP}, most notably we append the {\verb -fPIC } flag.
This flag creates \emph{position independent code}, in other words machine
instruction code that can execute properly regardless of where in memory it
resides. This will make one less register available on the x86 architecture.
While we cannot directly prove that this will affect performance of the
implementations, we do however suspect it. A person writing an optimization for
\ac{SUPERCOP} will have to balance his use of resources, among them registers,
to achieve maximum speed.  The loss of one register should therefore give a
performance penalty.

\section{Exclusion of mutable files}
...
compared to the time spent in the underlying C implementation. The reason for
...
\ref{tbl:tahoe:datadownloaded}). For these larger sizes the overhead caused by
...
If we look primarily at up- and download of large files, we see that the candidates
...
\subsection{Time spent not hashing} 

A strange part of our result is that the
time Tahoe-\ac{LAFS} spends not calling the hash function is not static for all
...
the cryptographic hash function in Tahoe-\ac{LAFS}. \autoref{tbl:tahoe:gain}
compares the performance gains for BLAKE, which performed the best in our
measurements, versus double \ac{SHA}-256, with the removal of overhead caused by our
...
We had hoped to contribute more to the Open Source project Tahoe-\ac{LAFS}, when
...
new implementations were prioritized. Future work should implement the support
for choosing hashing algorithm in a proper manner, which ensures backwards
compatibility.

Four of the candidates have been shown to perform better than the currently used
SHA-256 -- BLAKE, BlueMidnightWish, Shabal and Simd. The time it takes for
Tahoe-\ac{LAFS} to upload a one gigabyte file, could improve by $12,36\%$ if the
best performing \ac{SHA}-3 candidate, BLAKE, were to be used. One has to bear in
mind that \ac{SHA}-256 has been optimized and tweaked for a number of years, and
that these performance differences undoubtedly will continue to increase as new
and better implementations will be developed for the final chosen \ac{SHA}-3
algorithm.

Perhaps the most important finding in this project, is that the \ac{SUPERCOP}
benchmarking indeed drives the development of new and better implementations
forward, but since the \ac{SUPERCOP} \ac{API} does not conform to the \ac{NIST}
